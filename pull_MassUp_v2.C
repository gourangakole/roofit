...
pull_MassUp_v2()
{
  ...
  RooDataHist ds("ds","ds",mtop,Import(*Mu_topMass__TChannel_MassUp)) ;
  // Print number of events in dataset
  ds.Print();
  // Print binned dataset with default frame 
  RooPlot* frame = mtop.frame(Title("Top Mass(m_{top} = 178.5)")) ;
  
  ds.plotOn(frame) ;
  
  RooRealVar mean("mean","mean",165,160,180);
  RooRealVar sigma("sigma","sigma",8.6,5.0,30.0);
  RooGaussian gtop("gtop","gtop",mtop,mean,sigma);

  RooRealVar sigma1("sigma1","sigma1",21.0,19.0,24.0);
  RooRealVar sigma2("sigma2","sigma2",56.5,45.0,58.0);
