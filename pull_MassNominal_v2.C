...
void pull_MassNominal_v2()
{
  ...
  RooPlot* frame = mtop.frame(Title("Top Mass(m_{top} = 172.5)")) ;
  
  ds.plotOn(frame) ;
  
  RooRealVar mean("mean","mean",167.2,165,170);
  RooRealVar sigma("sigma","sigma",20.04,18.0,20.0);
  RooGaussian gtop("gtop","gtop",mtop,mean,sigma);

  RooRealVar sigma1("sigma1","sigma1",22.0,20.0,24.0);
  RooRealVar sigma2("sigma2","sigma2",53.5,48.0,57.0);
  RooBifurGauss bgtop("bgtop","bgtop",mtop,mean,sigma1,sigma2);

  RooRealVar frac0("frac0","frac0",0.25,0.1,0.5);
  ...
  */
  ...
  // pull 
  RooPlot* z1frame = mtop.frame(Title("Top Mass(m_{top} = 172.5)"));
  RooHist* hpull1 = frame->pullHist();
  z1frame->addPlotable(hpull1,"P");

  RooPlot* z2frame = mtop.frame(Title("Top Mass(m_{top} = 172.5)"));
 ...
 frame->Draw();
  ...
  c2->SaveAs("MassNominal_pull.root");
}
